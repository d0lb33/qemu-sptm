...
static bool hvf_vsh_fill(CPUState *cpu, uint64_t va, MMUAccessType access)
{
    ...
    for (unsigned i = 0; i < hvf_vsh.pages; i++) {
        if (hvf_vsh.page[i].va == va) {
            error_report("Virtual shadow access to existing alias "
                         "va=0x%" PRIx64 " access=%u", va, access);
            return false;
        }
    }
    ...
    descriptor = ipa | 0x703 | (UINT64_C(1) << 54); /* Privileged only. */
    ...
    hvf_vsh.page[hvf_vsh.pages++] = (HVFVirtualShadowPage) {
        .va = va, .pa = pa, .ipa = ipa, .flags = flags,
        .attrs = result.f.attrs,
    };
    hvf_vsh.generation++;
    error_report("Virtual shadow mapped va=0x%" PRIx64 " pa=0x%" PRIx64
                 " ipa=0x%" PRIx64 " rights=%u access=%u deps=%u",
                 va, pa, ipa, (unsigned)flags, access, hvf_vsh.dependencies);
    return true;
}
...
static bool hvf_vsh_guarded_store(CPUState *cpu, uint64_t far)
{
    ...
    if (address != far ||
        ...
        error_report("Virtual shadow guarded store denied by guest mapping "
                     "or range protection at 0x%" PRIx64, far);
        return false;
    }
    section = memory_region_find(arm_addressspace(cpu, result.f.attrs)->root,
...
static bool hvf_vsh_exception(CPUState *cpu)
{
    ...
    env->pc = elr;
    pstate_write(env, (spsr & ~UINT64_C(0xc)) | 8);
    aarch64_restore_sp(env, 2);
    unsigned ec = syn_get_ec(esr), fsc = esr & 0x3f;
    if ((ec == EC_DATAABORT + 1 || ec == EC_INSNABORT + 1) &&
        fsc >= 4 && fsc <= 7) {
        MMUAccessType access = ec == EC_INSNABORT + 1 ? MMU_INST_FETCH :
                               (esr & (1 << 6)) ? MMU_DATA_STORE :
                                                 MMU_DATA_LOAD;
        WITH_RCU_READ_LOCK_GUARD() {
            ok = hvf_vsh_fill(cpu, far, access);
        }
    } else if (ec == EC_DATAABORT + 1 && (esr & (1 << 6)) &&
               fsc >= 13 && fsc <= 15) {
        WITH_RCU_READ_LOCK_GUARD() {
