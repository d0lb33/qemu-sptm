/*
 * Apple AMX reference semantics, adapted for QEMU from corsix/amx
 * (https://github.com/corsix/amx, MIT License, Copyright (c) 2022 Peter
 * Cawley; see LICENSE.corsix in this directory).  The compute files in this
 * directory are the upstream emulate sources with only the include line
 * changed; loads and stores are reimplemented in ../amx_helper.c so that they
 * go through the guest MMU instead of host pointers.  AMX_VER is the
 * generation the machine advertises in AMXIDR_EL1 (amx_helper.c).
 */
#ifndef TARGET_ARM_TCG_AMX_EMULATE_H
#define TARGET_ARM_TCG_AMX_EMULATE_H
...
/* Cross-file symbols of the upstream sources (kept non-static upstream). */
float bf16_to_f32(uint32_t x);
uint16_t bf16_fma(uint16_t x, uint16_t y, uint16_t z);
...
#endif /* TARGET_ARM_TCG_AMX_EMULATE_H */
